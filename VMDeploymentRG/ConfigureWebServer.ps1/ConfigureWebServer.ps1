  ...
  Node ($MachineName)
  {
   ...
   #script block to download WebPI MSI from the Azure storage blob
	...
	Script DeployWebPackage
	{
        ...
        $WebClient.DownloadFile($WebDeployPackagePath,$destination)
        $ConnectionStringName = "DefaultConnection-Web.config Connection String"
        $ConnectionString = "Server=tcp:"+ "$DbServerName" + ".database.windows.net,1433;Database=" + "$DbName" + ";User ID=" + "$DbUserName" + "$DbServerName" + ";Password=" + "$DbPassword"+ ";Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;"
        $ConnectionString | Out-File -filepath C:\WindowsAzure\outfile.txt -append -width 200
        $Argument = '-source:package="C:\WindowsAzure\WebApplication.zip"' + ' -dest:auto,ComputerName="localhost",'+"username=$UserName" +",password=$Password" + ' -setParam:name="' + "$ConnectionStringName" + '"'+',value="' + "$ConnectionString" + '" -verb:sync -allowUntrusted' + '-setParam:name="IIS Web Application Name",value="Default Web Site"'
        #$Argument = -source:package="C:\WindowsAzure\WebApplication1.zip" -dest:auto,ComputerName="localhost",username="gauravbh",password=P@ssword3 -verb:sync -allowUntrusted -setParam:name="IIS Web Application Name",value="Default Web Site"
